...
YouTube **부분공개** 영상에서 다음을 순서대로 시연하면 됩니다.

1. 첫 화면의 학번·이름과 프로젝트 주제 설명
2. 회원가입 후 잘못된 로그인 / 정상 로그인 시연
3. 닉네임 설정, 루미볼트 지급, 로비 BGM과 전적 화면
4. `load_quiz_data()`의 `@st.cache_data` 코드와 JSON 재사용 이유 설명
5. 정답/오답 각각의 HP 변화, 피격 애니메이션, 겹쳐 재생되는 효과음
6. 실패 문구·재도전, 성공 문구·광운대 체육관 배지 및 획득 사운드

> 퀴즈 데이터와 캐시 코드는 설명을 위해 화면 녹화 중 에디터에서 잠깐 보여 주는 것이 좋습니다.
